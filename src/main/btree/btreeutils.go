...
func readNodeFromFile(offset int, file *os.File) (*TreeNode, error) {
	var err error
...
func encodeNode(node *TreeNode) []byte {
	...
	arr = append(arr, encodeInt(node.SelfOffset)...)
	arr = append(arr, encodeIntArray(BTREE_INDEX, node.Keys)...)
	arr = append(arr, encodeIntArray(BTREE_INDEX, node.RecordOffset)...)
	arr = append(arr, encodeIntArray(BTREE_INDEX+1, node.ChildOffset)...)
...
func decodeNode(bytes []byte) (*TreeNode, error) {
	...
	ansNode = TreeNode{
		...
		SelfOffset:   decodeInt(bytes[12:16]),
		Keys:         decodeIntArray(BTREE_INDEX, bytes[16:16+BTREE_INDEX*4]),
		RecordOffset: decodeIntArray(BTREE_INDEX, bytes[16+BTREE_INDEX*4:16+BTREE_INDEX*4*2]),
		ChildOffset:  decodeIntArray(BTREE_INDEX+1, bytes[16+BTREE_INDEX*4*2:16+BTREE_INDEX*4*2+(BTREE_INDEX+1)]),
	}
	return &ansNode, nil
}
...
func createNode(file *os.File) (*TreeNode, error) {
	newNode := TreeNode{
		SelfOffset:   -1,
		Keys:         make([]int, BTREE_INDEX),
		RecordOffset: make([]int, BTREE_INDEX),
		ChildOffset:  make([]int, BTREE_INDEX+1),
	}
...
func updateRootOffset(offset int, file *os.File) error {
	_, err := file.Seek(0, io.SeekStart)

	if err != nil {
		return err
	}
	_, err = file.Write(encodeInt(offset))
